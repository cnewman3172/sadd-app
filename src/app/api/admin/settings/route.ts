...
export async function PUT(req: NextRequest){
  ...
  let s = await prisma.setting.findUnique({ where:{ id:1 } });
  if (!s){ s = await prisma.setting.create({ data: { id:1, active:false } }); }
  const enableToggledOn = autoDisableEnabled && !s.autoDisableEnabled;
  const enableToggledOff = !autoDisableEnabled && s.autoDisableEnabled;
  const timeChanged = (autoDisableTime && autoDisableTime !== s.autoDisableTime) || (autoDisableTz && autoDisableTz !== s.autoDisableTz);
  const data: any = {
    autoDisableEnabled,
    autoDisableTime: autoDisableTime ?? s.autoDisableTime,
    autoDisableTz: autoDisableTz ?? s.autoDisableTz,
  };
  if (enableToggledOn || timeChanged){ data.scheduleSince = new Date(); }
  if (enableToggledOff){ data.scheduleSince = null; }
